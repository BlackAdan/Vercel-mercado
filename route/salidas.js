...
const Modelo = require('../models/SanMartin/Salidas')

const router = Router()

//obtener un dato
router.get('/api/salidas/:id', async (req, res) => {
    let {id} = req.params    
...
router.get('/api/salidas', (req, res) => {    
    Modelo.find()
    .exec()
    .then(x => res.status(200).send(x))
...
router.post('/api/salidas', async (req, res) => {
    await Modelo.insertMany(req.body)    
    res.json({'message':'Saved successful', "data":req.body})  
})

router.delete('/api/salidas/:id',async (req, res) => {
    let { id } = req.params;
    try{        
...
router.delete('/api/salidas/',async (req, res) => {
    try{        
...
router.put('/api/salidas/:id',async (req, res) => {
    let {id} = req.params
