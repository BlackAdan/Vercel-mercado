...
const Modelo = require('../models/SanMartin/Recepciones')

const router = Router()

//obtener un dato
router.get('/api/recepciones/:id', async (req, res) => {
    let {id} = req.params    
...
router.get('/api/recepciones', (req, res) => {    
    Modelo.find()
    .exec()
    .then(x => res.status(200).send(x))
...
router.post('/api/recepciones', async (req, res) => {
    await Modelo.insertMany(req.body)    
    res.json({'message':'Saved successful', "data":req.body})  
})

router.delete('/api/recepciones/:id',async (req, res) => {
    let { id } = req.params;
    try{        
...
router.delete('/api/recepciones/',async (req, res) => {
    try{        
...
router.put('/api/recepciones/:id',async (req, res) => {
    let {id} = req.params
