...
const Modelo = require('../models/SanMartin/ListaCortes')

const router = Router()

//obtener un dato
router.get('/api/listacortes/:id', async (req, res) => {
    let {id} = req.params    
...
router.get('/api/listacortes', (req, res) => {    
    Modelo.find()
    .exec()
    .then(x => res.status(200).send(x))
...
router.post('/api/listacortes', async (req, res) => {
    await Modelo.insertMany(req.body)    
    res.json({'message':'Saved successful', "data":req.body})  
})

router.delete('/api/listacortes/:id',async (req, res) => {
    let { id } = req.params;
    try{        
...
router.delete('/api/listacortes/',async (req, res) => {
    try{        
...
router.put('/api/listacortes/:id',async (req, res) => {
    let {id} = req.params
