...
const Modelo = require('../models/SanMartin/retiros')

const router = Router()

//obtener un dato
router.get('/api/retiros/:id', async (req, res) => {
    let {id} = req.params    
...
router.get('/api/retiros', (req, res) => {    
    Modelo.find()
    .exec()
    .then(x => res.status(200).send(x))
...
router.delete('/api/retiros/:id',async (req, res) => {
    let { id } = req.params;
    try{        
...
router.delete('/api/retiros/',async (req, res) => {
    try{        
...
router.put('/api/retiros/:id',async (req, res) => {
    let {id} = req.params
