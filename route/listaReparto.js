...
const Modelo = require('../models/SanMartin/ListaReparto')

const router = Router()

//obtener un dato
router.get('/api/tomas/luis/listareparto/:id', async (req, res) => {
    let {id} = req.params    
...
router.get('/api/listareparto', (req, res) => {    
    Modelo.find()
    .exec()
    .then(x => res.status(200).send(x))
...
router.post('/api/listareparto', async (req, res) => {
    await Modelo.insertMany(req.body)    
    res.json({'message':'Saved successful', "data":req.body})  
})

router.delete('/api/listareparto/:id',async (req, res) => {
    let { id } = req.params;
    try{        
...
router.delete('/api/listareparto/',async (req, res) => {
    try{        
...
router.put('/api/listareparto/:id',async (req, res) => {
    let {id} = req.params
